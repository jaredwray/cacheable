...
test("should get a value from the secondary store and respect its ttl", async () => {
	...
	const instance1 = new Cacheable({
		primary: instance1Primary,
		secondary: sharedSecondary,
	});
	const instance2 = new Cacheable({
		primary: instance2Primary,
		secondary: sharedSecondary,
	});
...
test("should not set in primary store if expired", async () => {
	...
	expect(
		result2,
		"result should not be placed in the primary store",
	).toBeUndefined();
});
