// biome-ignore-all lint/suspicious: legacy format
...
beforeAll(async () => {
	...
	s.get("/", (_request_: any, response_: any) => {
		response_.setHeader("cache-control", "max-age=60");
		response_.end("hi");
	});
	s.post("/", (_request_: any, response_: any) =>
		response_.status(201).end("hello"),
	);
});
afterAll(async () => {
	await s.close();
...
test("cacheableRequest does not cache response if request is aborted before receiving first byte of response", () => {
	void createTestServer().then((s) => {
		s.get("/delay-start", (_request_: any, response_: any) => {
			setTimeout(() => {
				response_.setHeader("cache-control", "max-age=60");
				response_.end("hi");
			}, 100);
...
test("cacheableRequest does not cache response if request is aborted after receiving part of the response", () => {
	void createTestServer().then((s) => {
		s.get("/delay-partial", (_request_, response_) => {
			response_.setHeader("cache-control", "max-age=60");
			response_.write("h");
			setTimeout(() => {
				response_.end("i");
